...
export class StorageSystem {
    ...
    public unlockChest(player: Player) {
        ...
        if(!chest || !(chest.info & 0x2000)) return;
        if(player.inventory.items.get(ItemType.LOCK_PICK)) {
            player.client.sendBinary(player.inventory.removeItem(ItemType.LOCK_PICK, 1));
            chest.info -= 0x2000;
        }
    }
    ...
    public giveWheat(player: Player, isShift: number) {
        ...
        if (windmill) {
            const count = Math.min(255, isShift ? 10 : 1, player.inventory.itemCount(ItemType.WHEAT), 255 - windmill.data[0]);

            windmill.data[0] += count;

            player.client.sendBinary(player.inventory.removeItem(ItemType.WHEAT, count));
        }
    }
