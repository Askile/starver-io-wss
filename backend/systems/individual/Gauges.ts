...
export class Gauges {
    ...
    public updateGauges() {
        ...
        if (this.old.hunger === 0 && this.hunger === 0) {
            ...
            this.player.healthSystem.damage(this.config.damage_food, ActionType.HUNGER);
        }

        if (this.old.cold === 0 && this.cold === 0) {
            this.player.reason = DeathReason.COLD;
            this.player.healthSystem.damage(this.player.winter ? this.config.damage_cold_winter : this.config.damage_cold, ActionType.COLD);
        }

        if (this.old.thirst === 0 && this.thirst === 0) {
            this.player.reason = DeathReason.WATER;
            this.player.healthSystem.damage(this.config.damage_water, ActionType.COLD);
        }

        if (this.old.oxygen === 0 && this.oxygen === 0) {
            this.player.reason = DeathReason.OXYGEN;
            this.player.healthSystem.damage(this.config.damage_oxygen, ActionType.COLD);
        }
