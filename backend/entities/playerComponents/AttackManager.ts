        ...
        if (this.isAttack && Date.now() - this.lastAttack >= 5e2) {
            ...
            for (const chunk of chunks) {
                for (const tile of chunk.tiles) {
                    if (!InventoryType[tile.resource as any]) continue;
                    ...
                    if (distance < totalRadius) {
                        ...
                        if(tile.count <= 0) {
                            this.player.client.sendBinary(new Uint8Array([ClientPackets.EMPTY_RES]))
                            continue;
                        }

                        this.player.client.sendBinary(
                            this.player.inventory.giveItem(InventoryType[tile.resource as any] as any, harvest)
                        );
